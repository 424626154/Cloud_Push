#import "CloudPushPlugin.h"

NSString *_isSuccessful = @"isSuccessful";
Boolean _initialized = NO;
Boolean _registered = NO;

@implementation CloudPushPlugin {
    // iOS 10通知中心
...
+ (void)registerWithRegistrar:(NSObject <FlutterPluginRegistrar> *)registrar {
    FlutterMethodChannel *channel = [FlutterMethodChannel
            methodChannelWithName:@"cloud_push"
                  binaryMessenger:[registrar messenger]];
    CloudPushPlugin *instance = [[CloudPushPlugin alloc] initWithRegistrar:registrar methodChannel:channel];
...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    if ([@"init" isEqualToString:call.method]) {
        ...
        [self syncBadgeNum:call result:result];
...
- (void)initPlugin {
    ...
    if(_registered && _initialized){
        [_methodChannel invokeMethod:@"initCloudChannelResult" arguments:@{
                @"isSuccessful": @YES,
                @"response": @"成功"
        }];
    }
...
- (void)syncBadgeNum:(FlutterMethodCall *)call result:(FlutterResult)result {
    NSNumber *target = (call.arguments == (id) [NSNull null]) ? @1 : call.arguments;
    [CloudPushSDK syncBadgeNum:target.unsignedIntegerValue withCallback:^(CloudPushCallbackResult *res) {
        if (res.success) {
            if (res.data == nil) {
